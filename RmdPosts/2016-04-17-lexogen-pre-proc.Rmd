...
categories: ["R", "NGS"]
date: 2016-04-17
title: "Analysis of Lexogen Quantseq Data: Part 1"
---
...
See [the Lexogen website](https://www.lexogen.com/quantseq-3mrna-sequencing/) and a [Nature Methods advertising feature](http://www.nature.com/nmeth/journal/v11/n12/full/nmeth.f.376.html) for more information.  This figure below is a schematic of the library preparation protocol:

![lib prep](/img/lexogen_schema.jpeg)

## Methods
The analysis is primary carried out using the `ShortRead` and `RSubread` [Bioconductor](www.bioconductor.org) packages for NGS analysis with `ggplot2` and the `cowplot` extension package used for visualisation.   The author thanks Martin Morgan, the `ShortRead` package author, for his assistance in developing this workflow.
...
We now generate 3 plots using the `qa_plots` function defined above:
*  Plot A depicts the proportion of reads with each nucleotide at positions 1-151.  It is evident that G's predominate towards the end of the reads, and A's increase up to around 60bp before reducing.  The read distribution seems to change markedly at around position 10.
*  Plot B depicts the average quality score at positions 1-151 across all reads.  Quality reduces towards the end of the read with an uptick towards the very end, and the first 10bp seem to have lower quality than the second 10bp.
*  Plot C is a histogram of read lengths: all reads are 151bp at this stage.
...
if(!file.exists(sqlite_path)) {
    ...
    ensembldb::ensDbFromGtf(gtf=gtf.file, path=ref_dir, outfile=sqlite_file, verbose=TRUE)
}
